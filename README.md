...
- 安装向导新增登录验证方式选择（可多选）；无验证模式需管理员环境密钥。
- 2FA（TOTP）完整前后端流程与管理面板开关。
...
- 当启用“无验证”模式时，管理员登录必须提供环境变量 `ADMIN_LOGIN_KEY`。  
  - Windows：在系统环境变量中新增 `ADMIN_LOGIN_KEY`，重启服务生效。  
  - Linux（systemd）：在服务文件中加入 `Environment=ADMIN_LOGIN_KEY=你的密钥`，然后执行 `systemctl daemon-reload` 并重启服务。
- 后端安装向导中的数据库密码不会在界面显示；若已保存，留空可保留当前密码，输入新值会覆盖。
- 在安装向导勾选 2FA 并保存时，后端控制台会输出管理员的 2FA 绑定信息（`secret`、`otpauth://` 以及终端二维码）。
- 你也可以手动重新输出二维码：在 `backend` 目录执行 `npm run print-2fa`（可选 `--username <name>`）。  
  - 默认不会重置已有 2FA；如需强制重生成请加 `--rotate`。
